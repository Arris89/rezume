...
?>
		...
		?>
			...
			<?if (array_key_exists($index+1, $arResult["arMap"]) && $arItem["LEVEL"] < $arResult["arMap"][$index+1]["LEVEL"]):?>
				...
				<li style="display: inline-block; list-style-type: none; margin-left: 25px;"><a href="<?=$arItem["FULL_PATH"]?>"><?=$arItem["NAME"]?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><?=$arItem["DESCRIPTION"]?><?}?></li>
