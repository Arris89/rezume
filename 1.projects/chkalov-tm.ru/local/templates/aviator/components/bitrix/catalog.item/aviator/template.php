...
//$this->addExternalJS($templateFolder . '/fancybox/jquery.fancybox-1.3.4.js');
//$this->addExternalCss($templateFolder . '/fancybox/jquery.fancybox-1.3.4.css');

$this->addExternalJS($templateFolder . '/owl.carousel.min.js');
$this->addExternalCss($templateFolder . '/owl.carousel.min.css');
...
if (isset($arResult['ITEM'])) {
    ...
    if (isset($item["PROPERTIES"]['MORE_PHOTO']['VALUE'])) {
        foreach ($item["PROPERTIES"]['MORE_PHOTO']['VALUE'] as $value) {
            $morePhotoLinks[] = CFile::GetPath($value);
        }
    }
    ...
    $templatePath = strtolower($arResult['TYPE']) . '/templates.php';
