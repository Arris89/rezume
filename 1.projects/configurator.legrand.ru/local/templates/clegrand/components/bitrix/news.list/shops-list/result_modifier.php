...
// получаем все магазины для карты
...
foreach ($arResult['ITEMS'] as &$arItem){
    $arItem['PROPERTIES'] = $arAllItems[$arItem['ID']]['PROPERTIES'];
}
unset($arItem);
...
foreach ($arAllItems as $arItem){
    if($arItem['PROPERTIES']['CONFIGURATOR_MAP']){
        ...
        if(in_array($arItem['ID'], $arResult['ELEMENTS'])){
            $_mapSizes['min_x'] = (!isset($_mapSizes['min_x']) || $coords[0] < $_mapSizes['min_x']) ? $coords[0] : $_mapSizes['min_x'];
            $_mapSizes['min_y'] = (!isset($_mapSizes['min_y']) || $coords[1] < $_mapSizes['min_y']) ? $coords[1] : $_mapSizes['min_y'];
            $_mapSizes['max_x'] = (!isset($_mapSizes['max_x']) || $coords[0] > $_mapSizes['max_x']) ? $coords[0] : $_mapSizes['max_x'];
            $_mapSizes['max_y'] = (!isset($_mapSizes['max_y']) || $coords[1] > $_mapSizes['max_y']) ? $coords[1] : $_mapSizes['max_y'];
        }
    }
}
$jsData['bounds'] = [[$_mapSizes['min_x'], $_mapSizes['min_y']],[$_mapSizes['max_x'], $_mapSizes['max_y']]];
