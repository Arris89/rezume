...
while($res = $ob->GetNext()){
    if($res['PROPERTY_FRAME_IMG_VERTICAL_VALUE'] > 0){
        $pic = $res['PROPERTY_FRAME_IMG_VERTICAL_VALUE'];
    }elseif($res['PROPERTY_FRAME_IMG_HORIZONTAL_VALUE'] > 0){
        $pic = $res['PROPERTY_FRAME_IMG_HORIZONTAL_VALUE'];
    }elseif($res['PROPERTY_FUNCTION_IMG_VALUE'] > 0){
        $pic = $res['PROPERTY_FUNCTION_IMG_VALUE'];
    }else{
        $pic = null;
    }
    ...
    // ресайз картинки
    if($pic){
        $pic = \CFile::ResizeImageGet($pic, array('width'=>168, 'height'=>168), BX_RESIZE_IMAGE_PROPORTIONAL_ALT , true)['src'];
    }
