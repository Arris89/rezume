    ...
    function initConfigurator() {
                ...
                if (drag.type !== 'frame') {
                    ...
                    for (var i = 0; i < slots.length; i++) {
                        ...
                        if ($(elem).css('display') === 'block') {
                            ...
                            var objHeight = drag.obj.height() * percent;
        ...
        function updateResult() {
            ...
            if (result.frame !== null) {
                ...
                $('.icon-text__text.deferred').attr("id", "deferred");
                $('#hrefdef').attr("href", "#popup-deferred");
            }
            else {
                /*деактивация кнопки Добавить в избранное*/
                $('.icon-text__text.deferred').attr("id", "");
                $('#hrefdef').attr("href", "");
            }
        ...
        function updateResultDeferred() {
            ...
            if (window.location.search == '') {
                var url = '?set_filter=y&' + $('#collect option[value="' + window.fcollection + '"]').attr('data-query');
            } else {
            ...
            $.post('/local/templates/clegrand/ajax/deferred.php', {
                ...
                console.log(datas);
                if (datas > 0) {
                    $('#def').attr('class', 'icon-text active icon-text_hover');
                }
            });
        }
        ...
        function updateBasketPrice() {
            ...
            $.post('/local/templates/clegrand/ajax/basketlist.php', {
                ...
                $('.header-basket__value').html('' + datab.NUM + itemForm + ' / ' + datab.SUM + ' руб.');
            });
        }
