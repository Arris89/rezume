...
$(function () {
    ...
    $(document).on('click', '.btn-collection-js', function () {
        ...
        if (typeof mechanismFilterIndependent['collectionName'] !== "undefined"
            ...
            mechanismFilterIndependent['dataFilter'] = mechanismFilterIndependent['collectionOption'].data('filter') + '=' + mechanismFilterIndependent['collectionName'];
            ...
            $('.' + mechanismFilterIndependent['class']['filterWrap'] + ' .select-js').find('option:selected').each(function () {
                ...
                if (typeof dataFilter !== 'undefined' && typeof dataValue !== 'undefined') {
    ...
    // const filterSet = new Set(['collection', 'frame-color', 'frame-material', 'mechanism-color', 'mechanism']);

    // $('select:not(.basic)').selectric({
    //     onInit: function(e, config) {
    //         const currentValue = $(this).val();
    //         const option = $(this).find('option:first-child');
    //         const wrapper = $(this).closest('.selectric-wrapper');
    ...
    //         }
    //         checkFirstOption($(this));
    //         if (filterSet.has($(select).attr('name'))) {
    //             if ($('select[name="collection"]').find('option:selected').val() == 'all') {
    //                 delete mechanismFilterIndependent['dataFilter'];
    //                 delete mechanismFilterIndependent['dataQuery'];
    //             }
    ...
    function checkCountElementsAjax(params) {
        ...
        $.ajax({
            ...
            error: function () {
                console.log('Error updating filter for catalog!');
            },
                    ...
                    if (countElements > 0) {
                        ...
                        if (params.type == 'mechanism') {
                            ...
                            var selectedColor = params.color.find('option:selected'),
                                ...
                                colorCatalog = selectedColor.data('filter') + '=' + selectedColor.val(),
                                mechanismCatalog = selectedMechanism.data('filter') + '=' + selectedMechanism.val();
                        ...
                        if (params.type == 'frame') {
                            var selectedColorFrame = params.color.find('option:selected'),
                                ...
                                colorCatalogFrame = selectedColorFrame.data('filter') + '=' + selectedColorFrame.val(),
                                materialCatalogFrame = selectedMaterialFrame.data('filter') + '=' + selectedMaterialFrame.val();
    ...
    function getChangeSelectMechanism() {
        ...
        //configuratorCatalogAjax(mechanism);
        ...
        //configuratorFilterAjax(mechanism);
            ...
            complete: function (jqXHR, textStatus) {
                ...
                if (jqXHR.status == 200 && textStatus == 'success') {
                    ...
                    $('.' + params.class.filterWrap + ' .select-js').find('option').each(function () {
                        ...
                        if (this.getAttribute('data-filter') != 'arrFilter_60') {
