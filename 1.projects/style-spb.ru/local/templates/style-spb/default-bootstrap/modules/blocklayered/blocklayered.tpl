							...
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										{assign var='new_link' value=$value.link}
										{if $value.link=='https://style-spb.ru/12-coats#kategorii-demisezonnye_palto'}
											{assign var='new_link' value='https://style-spb.ru/20-demisezonnye-palto'}
										{/if}
										{if $value.link=='https://style-spb.ru/12-coats#kategorii-uteplennye_palto'}
											{assign var='new_link' value='https://style-spb.ru/24-uteplennye-palto'}
										{/if}
										{if $value.link=='https://style-spb.ru/12-coats#kategorii-dlinnye_palto'}
											{assign var='new_link' value='https://style-spb.ru/26-dlinnye-palto'}
										{/if}
										{if $value.link=='https://style-spb.ru/12-coats#kategorii-korotkie_palto'}
											{assign var='new_link' value='https://style-spb.ru/27-korotkie-palto'}
										{/if}
										{if $value.link=='https://style-spb.ru/12-coats#kategorii-klassicheskie_palto'}
											{assign var='new_link' value='https://style-spb.ru/28-klassicheskie-palto'}
										{/if}
										{if $value.link=='https://style-spb.ru/13-kostyumy#kategorii-bryuki'}
											{assign var='new_link' value='https://style-spb.ru/14-bryuki'}
										{/if}
										{if $value.link=='https://style-spb.ru/13-kostyumy#kategorii-zhakety'}
											{assign var='new_link' value='https://style-spb.ru/15-zhakety'}
										{/if}
										{if $value.link=='https://style-spb.ru/13-kostyumy#kategorii-platya'}
											{assign var='new_link' value='https://style-spb.ru/16-platya'}
										{/if}
										{if $value.link=='https://style-spb.ru/13-kostyumy#kategorii-yubki'}
											{assign var='new_link' value='https://style-spb.ru/17-yubki'}
										{/if}
										{if $value.link=='https://style-spb.ru/13-kostyumy#kategorii-bluzy'}
											{assign var='new_link' value='https://style-spb.ru/18-bluzy'}
										{/if}
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-12">
											...
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" data-rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
												...
												{else}
												<a href="{$new_link}"{if $value.rel|trim != ''} data-rel="{$value.rel}"{/if}> {$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
												{/if}
											</label>
										</li>
										{/if}
