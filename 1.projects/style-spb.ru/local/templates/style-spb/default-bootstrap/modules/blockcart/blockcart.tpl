...
{/if}
	<a href="tel:88005050517" style="display:block;position:absolute;right:20px;top:24px;font-size:18px;z-index:9">8 800 505 05 17</a>
	<div class="shopping_cart" style="padding-top:55px">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			...
			<span class="ajax_cart_total{if $cart_qties == 0} unvisible{/if}">
				{if $cart_qties > 0}#header .shopping_cart
					{if $priceDisplay == 1}
					...
					{/if}.shopping_cart > a:first-child
				{/if}
			</span>
