...
?>
		...
		$elementId = $APPLICATION->IncludeComponent('bitrix:catalog.element', 'style', $componentElementParams,
			$component
		);
			...
			if (!empty($recommendedData))
			{
				if (!empty($recommendedData['IBLOCK_LINK']) || !empty($recommendedData['ALL_LINK']))
				{
					?>
							...
							<?
							$APPLICATION->IncludeComponent('bitrix:catalog.recommended.products', 'style', array(
									'ID' => $elementId,
									'IBLOCK_ID' => $arParams['IBLOCK_ID'],
									'IBLOCK_TYPE' => $arParams['IBLOCK_TYPE'],
