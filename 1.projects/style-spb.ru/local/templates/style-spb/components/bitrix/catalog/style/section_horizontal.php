...
/*вывод количества товаров на страницу*/
...
if (isset($_SESSION['sortcat'])) {
    ...
    if ($_SESSION['sortcat'] == 'NAME') {
        $sortCat = $_SESSION['sortcat'];
        $orderCat = $_SESSION['ordercat'];
    } else {
        $sortCat = 'PROPERTY_' . "$_SESSION[sortcat]";
        $orderCat = $_SESSION['ordercat'];
    }

} else {
    $sortCat = $arParams["ELEMENT_SORT_FIELD"];
    $orderCat = $arParams["ELEMENT_SORT_ORDER"];
}
...
<div id="center_column" class="qwe11 center_column col-xs-12 col-sm-12">
    <?
    $intSectionID = $APPLICATION->IncludeComponent("bitrix:catalog.section", "style", array(
        "IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
        "IBLOCK_ID" => $arParams["IBLOCK_ID"],
        "ELEMENT_SORT_FIELD" => $sortCat,
        "ELEMENT_SORT_ORDER" => $orderCat,
